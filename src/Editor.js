...
import { markIsActive, nodeIsActive, getMarkAttrs, actorIsActive } from 'tiptap-utils'
import {
  injectCSS,
  camelCase,
  Emitter,
    ...
    this.activeNodes = Object
      ...
      .reduce((nodes, [name, node]) => {
        if (name === 'paragraph') {
          return {
            ...nodes,
            [name]: (attrs = {}, pos) => actorIsActive(this.state, node, attrs, pos),
          }   
        } else {
          return {
            ...nodes,
    ...
    return Object
      ...
      .reduce((types, [name, value]) => {
        if (name === 'paragraph') {
          return {
            ...types,
            [name]: (attrs = {}, pos) => value(attrs, pos),
          }
        } else {
          return {
            ...types,
